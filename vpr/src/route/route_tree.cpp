...
RouteTreeNode::RouteTreeNode(const RouteTreeNode& rhs)
    : RouteTreeNode(rhs.inode, rhs.parent_switch, vtr::nullopt) {
...
void RouteTreeNode::print_x(int depth) const {
    ...
    if (parent) {
        VTR_LOG("parent: %d \t parent_switch: %d", parent.value().inode, parent_switch);
...
RouteTree::RouteTree(RRNodeId _inode)
    : _root(RouteTreeNode(_inode, RRSwitchId(OPEN), vtr::nullopt)) {
...
RouteTree::RouteTree(ParentNetId _inet)
    : _root(RouteTreeNode(get_source_of_net(_inet), RRSwitchId(OPEN), vtr::nullopt)) {
...
void RouteTree::reload_timing(vtr::optional<RouteTreeNode&> from_node) {
    ...
    load_new_subtree_R_upstream(from_node.value());

    // Propagate C_downstream up from new subtree sinks to subtree root
    load_new_subtree_C_downstream(from_node.value());

    // Propagate C_downstream up from the subtree root
    RouteTreeNode& unbuffered_subtree_rt_root = update_unbuffered_ancestors_C_downstream(from_node.value());
...
RouteTree::update_unbuffered_ancestors_C_downstream(RouteTreeNode& from_node) {
    ...
    vtr::optional<RouteTreeNode&> last_node = from_node;
    vtr::optional<RouteTreeNode&> parent_rt_node = from_node.parent;
...
RouteTree::add_subtree_from_heap(t_heap* hptr, int target_net_pin_index, bool is_flat) {
    ...
    RouteTreeNode& found_rt_node = _rr_node_to_rt_node.at(new_inode).value();
    ...
    for (int i = new_branch_inodes.size() - 1; i >= 0; i--) {
        RouteTreeNode& new_node = last_node.value().emplace_child(new_branch_inodes[i], new_branch_iswitches[i], last_node);
        _rr_node_to_rt_node[new_node.inode] = new_node;
    ...
    for (RRNodeId rr_node : main_branch_visited) {
        add_non_configurable_nodes(_rr_node_to_rt_node.at(rr_node).value(), false, all_visited, is_flat);
    }
...
void RouteTree::add_non_configurable_nodes(RouteTreeNode& rt_node,
    ...
    for (int iedge : rr_graph.non_configurable_edges(rr_node)) {
        ...
        RouteTreeNode& new_node = rt_node.emplace_child(to_rr_node, edge_switch, rt_node);
        _rr_node_to_rt_node[new_node.inode] = new_node;
...
RouteTree::prune(CBRR& connections_inf, std::vector<int>* non_config_node_set_usage) {
    ...
    VTR_ASSERT_MSG(rr_graph.node_type(_root.inode) == SOURCE, "Root of route tree must be SOURCE");

    VTR_ASSERT_MSG(route_ctx.rr_node_route_inf[size_t(_root.inode)].occ() <= rr_graph.node_capacity(_root.inode),
...
RouteTree::prune_x(RouteTreeNode& rt_node, CBRR& connections_inf, bool force_prune, std::vector<int>* non_config_node_set_usage) {
    ...
    rt_node.remove_child_if([&](auto& child) {
        ...
        if (child_maybe.has_value()) { // Not pruned
            ...
            _rr_node_to_rt_node.erase(child.inode);
            return true;
        }
    });
    ...
    if (rr_graph.node_type(rt_node.inode) == SINK) {
        ...
        VTR_ASSERT(rt_node.child_nodes().size() == 0);
